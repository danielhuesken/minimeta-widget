<?PHP
//active plugins only File names in lower case
$plugins=get_option('active_plugins');
for ($i=0;$i<sizeof($plugins);$i++) {
 $path=pathinfo($plugins[$i]);
 $plugins[$i]=strtolower($path["basename"]);
}

//adminlinks
$adminlinks[1]['menu'] =  __('Dashboard'); //menu group
$adminlinks[1][5] = array(__('Dashboard'), '1', 'index.php'); //menu link
if (in_array("wp-serverinfo.php",$plugins)) $adminlinks[1][10] = array(__('WP-ServerInfo', 'wp-serverinfo'), 1, 'index.php?page=wp-serverinfo/wp-serverinfo.php'); //menu link
if (in_array("wp-stats.php",$plugins)) $adminlinks[1][11] = array(__('WP-Stats', 'wp-stats'), 1, 'index.php?page=wp-stats/wp-stats.php'); //menu link
if (in_array("wp-useronline.php",$plugins)) $adminlinks[1][12] = array(__('WP-UserOnline', 'wp-useronline'), 1, 'index.php?page=wp-useronline/useronline.php'); //menu link
...
if (in_array("sf-control.php",$plugins)) $adminlinks[10][50] =  array(__('Simple Forum', 'sforum'), 8, 'edit.php?page=simple-forum/sf-admin.php');
if (in_array("wp-ban.php",$plugins)) $adminlinks[10][51] =  array(__('Ban', 'wp-ban'), 'manage_options', 'edit.php?page=wp-ban/ban-options.php');
...
if (in_array("akismet.php",$plugins)) $adminlinks[15][30] = array(sprintf(__('Akismet Spam (%s)'), akismet_spam_count()), 'moderate_comments', 'edit-comments.php?page=akismet-admin');
...
//if (in_array("sphere-related-content.php",$plugins)) $adminlinks[30][10] = array(__('Sphere Configuration'), 'manage_options', 'plugins.php?page=sphere-related-content/sphere-related-content.php');
...
//if (in_array("akismet.php",$plugins)) $adminlinks[30][20] = array(__('Akismet Configuration'), 'manage_options', 'plugins.php?page=akismet-key-config');
...
if (in_array("wp-pagenavi.php",$plugins)) $adminlinks[40][50] = array(__('PageNavi', 'wp-pagenavi'), 'manage_options', 'options-general.php?page=wp-pagenavi/pagenavi-options.php');
if (in_array("wp-postviews.php",$plugins)) $adminlinks[40][51] = array(__('Post Views', 'wp-postviews'), 'manage_options', 'options-general.php?page=wp-postviews/postviews-options.php');
if (in_array("wp-print.php",$plugins)) $adminlinks[40][52] = array(__('Print', 'wp-print'), 'manage_options', 'options-general.php?page=wp-print/print-options.php');
if (in_array("wp-stats.php",$plugins)) $adminlinks[40][53] = array(__('Stats', 'wp-stats'), 'manage_options', 'options-general.php?page=wp-stats/stats-options.php');
if (in_array("wp-sticky.php",$plugins)) $adminlinks[40][54] = array(__('Sticky', 'wp-sticky'), 'manage_options', 'options-general.php?page=wp-sticky/sticky-options.php');
if (in_array("wp-useronline.php",$plugins)) $adminlinks[40][55] = array(__('Useronline', 'wp-useronline'), 'manage_options', 'options-general.php?page=wp-useronline/useronline-options.php');
if (in_array("wp-cache.php",$plugins)) $adminlinks[40][56] = array('WP-Cache', 5, 'options-general.php?page=wp-cache/wp-cache.php');
if (in_array("all_in_one_seo_pack.php",$plugins)) $adminlinks[40][57] = array(__('All in One SEO', 'all_in_one_seo_pack'), 'manage_options', 'options-general.php?page=all-in-one-seo-pack/all_in_one_seo_pack.php');
if (in_array("get-recent-comments.php",$plugins)) $adminlinks[40][58] = array('Recent Comments', 8, 'options-general.php?page=get-recent-comments.php');
if (in_array("sitemap.php",$plugins)) $adminlinks[40][59] = array(__('XML-Sitemap','sitemap'), 'administrator', 'options-general.php?page=sitemap.php');
if (in_array("podpress.php",$plugins)) {
...
if (in_array("nggallery.php",$plugins)) {
...
if (in_array("wp-dbmanager.php",$plugins)) {
...
if (in_array("wp-downloadmanager.php",$plugins)) {
...
if (in_array("wp-email.php",$plugins)) {
...
if (in_array("wp-polls.php",$plugins)) {
...
if (in_array("wp-postratings.php",$plugins)) {
